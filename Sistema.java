...
public class Sistema {
	...
	public class MemoryManager {
		...
		public int partitionSize; // Tamanho da partição
		public Memory memory; //Memória
		public int partitionNumber; // Qtd de partições (M/P)
		public boolean[] frames;

		public MemoryManager(int partitionSize, Memory memory){
			this.partitionSize = partitionSize; // 64
			this.memory = memory; //memory.m = 1024
			this.partitionNumber = memory.tamMem/partitionSize; // 16
			this.frames = new boolean[partitionNumber];
			for (int i = 0; i < frames.length; i++) { frames[i] = true; } // seta todas como livre
		}

		public int allocate(int wordNumber){
				// lanca execacao caso nao consegue alocar
				if (wordNumber > partitionSize) {
					return -1;
				}
				...
				return -1; // retorna numeroParticao
		}

		public void deallocate(int partition){
			frames[partition] = false; // marca partição como livre
		}
	...
	public class ProcessManager {
		...
		HashMap<Integer,PCB> pcb = new HashMap<>();
		int id = 0;
		...
		public class PCB {
			...
			public int particaoUtilizada;
			public int id;
			public int pc;

			public PCB(int particaoUtilizada, int id, int pc){
				this.particaoUtilizada = particaoUtilizada;
				this.id = id;
				this.pc = pc;
			}
...
//  Retorna true
		int criaProcesso(Word[] programa) {

			// setaParticao
			int partitionStart = vm.memoryManager.allocate(programa.length);
			...
			// cria PCB
			int id = vm.processManager.gerarId();
			vm.processManager.pcb.put(id,new PCB(partitionStart, id, 0));
			
			// Carrega o programa
			int traducao = traduzir(0, partitionStart);
			if(traducao == -1){
				System.out.println("Falha na tradução de endereço");
				return -1;
			} 

			int j = 0;
			for (int i = traducao; i < programa.length; i++) {
				vm.memoryManager.memory.m[i].opc = programa[j].opc;
				vm.memoryManager.memory.m[i].r1 = programa[j].r1;
				vm.memoryManager.memory.m[i].r2 = programa[j].r2;
				vm.memoryManager.memory.m[i].p = programa[j].p;
				j++;
			}
		...
		boolean desalocaProcesso(int id){
			// desaloca a memória com id
			vm.memoryManager.deallocate(vm.processManager.pcb.get(id).particaoUtilizada);
			// remove da fila
			vm.processManager.pcb.remove(id);
			return true;
		}
	...
	public class VM {
		...
		public ArrayList<Integer> readyPcb = new ArrayList<>();
        public CPU cpu;    
        ...
        public VM(InterruptHandling ih, SysCallHandling sysCall){   
	     ...
	     // cria memória
		     tamMem = 1024;
			 tamPart = 64;
			 memoryManager = new MemoryManager(tamPart, new Memory(tamMem));
			 processManager = new ProcessManager();
	  	 // cria cpu
			 cpu = new CPU(memoryManager.memory,ih,sysCall, true);                   // true liga debug
	    }	
	}
    // ------------------- V M  - fim ------------------------------------------------------------------------
	// -------------------------------------------------------------------------------------------------------
	...
	private int traduzir(int endLogico, int particao){
			if (endLogico < 0 || endLogico > 63) {
				return -1;
			}
 			return particao*vm.tamPart + endLogico;
	}
	...
	private int loadProgram(Word[] p) {
		...
		return vm.processManager.criaProcesso(p);

		/*int partitionStart = vm.memoryManager.allocate(p.length);
		...
		int j = 0;
		for (int i = traducao; i < p.length; i++) {
			vm.memoryManager.memory.m[i].opc = p[j].opc;
			vm.memoryManager.memory.m[i].r1 = p[j].r1;
			vm.memoryManager.memory.m[i].r2 = p[j].r2;
			vm.memoryManager.memory.m[i].p = p[j].p;
			j++;
		}*/
	}
	...
	private void loadAndExec(Word[] p){
		loadProgram(p);    // carga do programa na memoria
				...
				vm.memoryManager.memory.dump(0, p.length);            // dump da memoria nestas posicoes				
		vm.cpu.setContext(0, vm.tamMem - 1, 0);      // seta estado da cpu ]
				System.out.println("---------------------------------- inicia execucao ");
		vm.cpu.run();                                // cpu roda programa ate parar	
				System.out.println("---------------------------------- memoria após execucao ");
				vm.memoryManager.memory.dump(0, p.length);            // dump da memoria com resultado
	}

	private void exec(int id){
		PCB process = vm.processManager.pcb.get(id);
				System.out.println("---------------------------------- programa carregado na memoria");
				vm.memoryManager.memory.dump(0, p.length);            // dump da memoria nestas posicoes				
		vm.cpu.setContext(0, vm.tamMem - 1, 0);      // seta estado da cpu ]
				System.out.println("---------------------------------- inicia execucao ");
